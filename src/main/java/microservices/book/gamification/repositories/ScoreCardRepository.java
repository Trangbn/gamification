...
public interface ScoreCardRepository extends CrudRepository<ScoreCard, Long> {
    ...
    /**
     * Retrieves a list of the {@link LeaderBoardRow}s representing the Leader Board of users and their total score
     * @return the leader board, sorted by highest score first
     */
    @Query(" SELECT NEW microservices.book.gamification.domain.LeaderBoardRow(s.userId, SUM(s.score))" + "FROM microservices.book.gamification.domain.ScoreCard s " + " GROUP BY s.userId ORDER BY SUM(s.score) DESC " )
    List<LeaderBoardRow> findFirst10();

    /**
     * Retrieves all the ScoreCards for a given users. identified by his user id.
